function organizeSeparateTextures_STS(pathWORK,nPatient,R_mat,scale_cell,algo_cell,Ng_mat)
% -------------------------------------------------------------------------
% function organizeSeparateTextures(pathWORK,nPatient,scan_cell,R_mat,scale_cell,algo_cell,Ng_mat)
% -------------------------------------------------------------------------
% DESCRIPTION: 
% This function organizes SEPARATE texture features computed for all 
...
% for more details.
%
% Textures are organized in three cells: 'textures_PET', 'textures_T1' and 
% 'textures_T2FS', where each cell entry organizes the data for all 
...
% INITIALIZATION
scans = {'PET','T1','T2FS'}; % For now on, T2-weighted fat-saturated and STIR scans are combined in the same texture category: T2FS (T2-weighted fat-suppression)
nScans = numel(scans);
suffix = ['_TEXT'];
                ...
                for n = 1:numel(Ng_mat)
                    ...
                    for t = 1:nType-1 % Last field is 'parameters'
                        ...
                        for f = 1:nFeature
                            ...
                            text{r,s,a,n}.(nameType{t}).(nameFeature{f}).Spearman = corr(data,outcome,'type','Spearman');
                        end
                    end
                end
            end
