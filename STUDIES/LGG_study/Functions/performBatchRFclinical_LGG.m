function performBatchRFclinical_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% function performBatchRFclinical_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% DESCRIPTION: 
% This function performs prediction estimation using out-of-bag estimates
% for random forests constructed using (CLINICAL) features.
% -------------------------------------------------------------------------
% INPUTS:
...
for o = 1:nOutcomes
    ...
    fprintf(fid,['X = dataStruct.(nameOutcome).data(:,(nVas+nText+1):end);\n']);
    fprintf(fid,['Y = dataStruct.(nameOutcome).outcome;\n']);
    fprintf(fid,['cat = dataStruct.(nameOutcome).categories((nVas+nText+1):end);\n']);
...
cd(pathRF), save('tableSummary_Clinical','tab')
