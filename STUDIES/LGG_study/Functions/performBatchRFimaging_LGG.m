function performBatchRFimaging_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% function performBatchRFimaging_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% DESCRIPTION: 
% This function performs prediction estimation using out-of-bag estimates
% for random forests constructed using (TEXTURES + VASARI) features.
% -------------------------------------------------------------------------
% INPUTS:
...
mkdir('batchLog_RFimaging'), cd('batchLog_RFimaging'), pathBatch = pwd;
...
for o = 1:nOutcomes
    ...
    fprintf(fid,['X = dataStruct.(nameOutcome).data(:,1:(nVas+nText));\n']);
    fprintf(fid,['Y = dataStruct.(nameOutcome).outcome;\n']);
    fprintf(fid,['cat = dataStruct.(nameOutcome).categories(1:(nVas+nText));\n']);
...
cd(pathRF), save('tableSummary_Imaging','tab')
