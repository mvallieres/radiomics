function performBatchRFtextures_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% function performBatchRFtextures_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% DESCRIPTION: 
% This function performs prediction estimation using out-of-bag estimates
% for random forests constructed using (TEXTURE) features.
% -------------------------------------------------------------------------
% INPUTS:
...
mkdir('batchLog_RFtextures'), cd('batchLog_RFtextures'), pathBatch = pwd;
...
for o = 1:nOutcomes
    ...
    fprintf(fid,['X = dataStruct.(nameOutcome).data(:,(nVas+1):(nVas+nText));\n']);
    fprintf(fid,['Y = dataStruct.(nameOutcome).outcome;\n']);
    fprintf(fid,['cat = dataStruct.(nameOutcome).categories((nVas+1):(nVas+nText));\n']);
...
cd(pathRF), save('tableSummary_Textures','tab')
