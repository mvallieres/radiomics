function performBatchRF_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% function performBatchRF_LGG(pathRF,nTrees,seed,matlabPATH)
% -------------------------------------------------------------------------
% DESCRIPTION: 
% This function performs prediction estimation using out-of-bag estimates
% for random forests constructed using (TEXTURES + VASARI + CLINICAL)
% features.
% -------------------------------------------------------------------------
% INPUTS:
...
for o = 1:nOutcomes
    ...
    fprintf(fid,['X = dataStruct.(nameOutcome).data;\n']);
    fprintf(fid,['Y = dataStruct.(nameOutcome).outcome;\n']);
    fprintf(fid,['cat = dataStruct.(nameOutcome).categories;\n']);
...
cd(pathRF), save('tableSummary_Combined','tab')
