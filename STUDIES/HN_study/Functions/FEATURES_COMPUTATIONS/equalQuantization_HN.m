function [ROIonlyEqual,levels] = equalQuantization_HN(ROIonly,Ng)
% -------------------------------------------------------------------------
...
ROIonly = ROIonly./max(ROIonly(:)); % MISTAKE HERE WHEN I FIRST COMPUTED FEATURES. SEE IF IT AFFECTS RESULTS BEFORE GOING TO CORRECT VERSION ON GITHUB.
qmax = max(ROIonly(:));
qmin = min(ROIonly(:));
