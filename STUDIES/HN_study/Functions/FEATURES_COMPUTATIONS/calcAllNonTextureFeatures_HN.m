function calcAllNonTextureFeatures_HN(pathData,pathNonText,namePT,nameCT,nameROI,featType)
% -------------------------------------------------------------------------
...
for i = 1:nPatient
    ...
    [contourVect] = findContours_HN(sData,{nameROI{i}},'PET');
    ...
    [percentInactive] = getPercentInactive(SUVmap,0.01);
    ...
    % Volume, Size, Solidity, Eccentricity, Compactness
    load(nameCT{i}) % Variable 'sData' now in MATLAB Workspace
    mask = zeros(size(sData{2}.scan.volume.data));
    [contourVect] = findContours_HN(sData,{nameROI{i}},'CT');
    for c = 1:numel(contourVect)
        temp = zeros(size(sData{2}.scan.volume.data));
        [~,ROImask,bb] = getROI(sData,contourVect(c));
        temp(bb(1,1):bb(1,2),bb(2,1):bb(2,2),bb(3,1):bb(3,2)) = ROImask;
    ...
    fprintf('DONE\n')
    fprintf(['COMPUTING ''COMPACTNESS'' of ',nameCT{i},' ... '])
    [compactness] = getCompactness(ROIonly,pixelW,sliceS); nonTextures.Compactness = compactness;
    fprintf('DONE\n')
    fprintf('\n')
