function calcAllAertsFeatures_batchHN(pathData,pathText,namePT,nameCT,nameROI,outcomes,featType,nBatch,matlabPATH)
% -------------------------------------------------------------------------
...
scans = {'PT','CT','CTorig'}; nScans = numel(scans);
...
for scan = 1:nScans
    ...
    for p = 1:nPatient
        ...
        if strcmp(scans{scan},'CTorig')
            if exist([namePatientScan,'_',featType,'_sign_orig.mat'],'file')
                load([namePatientScan,'_',featType,'_sign_orig']) % Variable 'signature' is now in MATLAB workspace
            else
                load([namePatientScan,'_','GTVp','_sign_orig']) % Variable 'signature' is now in MATLAB workspace
            end
        else
