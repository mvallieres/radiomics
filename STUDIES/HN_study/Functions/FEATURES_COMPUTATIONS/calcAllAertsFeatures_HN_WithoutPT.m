function calcAllAertsFeatures_HN_WithoutPT(pathData,pathText,nameCT,nameROI,featType)
% -------------------------------------------------------------------------
% function calcAllSeparateTextures_HN(pathData,pathText,nameCT,nameROI,featType,scale_mat,Ng_mat)
% -------------------------------------------------------------------------
% DESCRIPTION: 
...
%             --> Ex: {'GTV';'GTV-P'}
% 5. featType: Either 'GTVp' for primary GTV, or 'GTVtot' for primary GTV +
%              nodal GTVs
% 6. scale_mat: Array vector specifying the different 'Scale' values to test.
%               --> Ex: [1,2,3,4,5]
% 7. Ng_mat: Array vector specifying the different 'Ng' values to test.
%            --> Ex: [8,16,32,64]
% -------------------------------------------------------------------------
...
% INITIALIZATION
scans = {'CT'}; nScans = numel(scans); names = {nameCT};
scanTypes = {'CT'};
...
for i = 1:nPatient
    for j = 1:nScans
        ...
        if mod(sizeROI(1),2) % To facilitate the wavelet transform computations
            a = a + 1;
        end
        if mod(sizeROI(2),2)
            b = b + 1;
        ...
        [signature] = getAertsSign(vol,mask,spatialRef,scanType);
        ind = strfind(names{j}{i},'.'); namePatientScan = names{j}{i}(1:ind(1)-1);
        cd(pathText), save([namePatientScan,'_',featType,'_sign'],'signature')
        time = toc(tStart);
