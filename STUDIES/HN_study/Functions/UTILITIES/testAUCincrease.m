function [p,CI] = DeLongTest(X1,X2,Y)
% -------------------------------------------------------------------------
...
LSL = L*S*L';
val_chi2 = (auc*L')*(inv(LSL))*(L*auc');
df_chi2 = rank(LSL);
if df_chi2 == 1
    val_chi2 = val_chi2*sqrt(2); % Doubling the z-score since we are looking at changes in one direction (z-score = chi^2 for df = 1)
end
