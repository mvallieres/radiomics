function [contourVect] = findContours(sData,cellName)
% -------------------------------------------------------------------------
% function [contourVect] = findContours(sData,cellName)
% -------------------------------------------------------------------------
% DESCRIPTION: 
...
for i = 1:nEntry
    ...
    if isempty(indComma)
        ...
        for j = 1:numel(indComma)-1
            contourNames = [contourNames,string((indComma(j)+1):((indComma(j)-1)))];
        end
    end
end
...
for i = 1:nContour
    for j = 1:nContourData
        ...
        indSep = strfind(name,'--');
        if strcmp(name((indSep(end)+2):end),contourNames{i})
            contourVect(i) = j;
            break
        end
    end
