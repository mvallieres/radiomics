function [signature] = getAertsSign_MatlabBased_HLH(vol,mask,spatialRef,scanType)

% PARAMETERS
if strcmp(scanType,'CT')
    res = [1,1,3]; % In (Aerts,2014), all features were calculated from 1X1X3 raw images
    levels = 0:25:3000; % Division of quantized bins in (Aerts,2014) for texture analysis
elseif strcmp(scanType,'PET') % To be tested in future work
...
% STEP 4: CALCULATE FEATURE 4 (HLH --> H: anterior-posterior, L: left-right, H: superior-inferior)
sizeV = size(ROIbox);
if mod(sizeV(1),2)
    sizeV(1) = sizeV(1) + 1;
end
