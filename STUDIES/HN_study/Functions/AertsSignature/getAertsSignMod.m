function [signature] = getAertsSignMod(vol,mask,spatialRef,scanType)

% PARAMETERS
if strcmp(scanType,'CT')
    res = [1,1,1]; % In (Aerts,2014), all features were calculated from 1 X 1 X 3 mm^3 raw images
...
compactness = volume/(sqrt(pi)*surface^(3/2)); % MODIFICATION: REPLACING 2/3 BY 3/2
...
[GLN] = getGLN_AertsMod(ROIquant,newLevels); % MODIFICATION: NO AVERAGING OF GLRLM + NORMALIZED GLN
signature(3) = GLN;
...
[GLN] = getGLN_AertsMod(ROIquant,levels); % MODIFICATION: NO AVERAGING OF GLRLM + NORMALIZED GLN
signature(4) = GLN;
