function [fSet] = featureSetReductionTime_HN(pathMINE,fSetNameType,outcome,setSize,nonTextStruct,textCells,textCellsName,paramAll,paramUsed,baseline,alpha,delta,nBoot,seed,batchNum)
% -------------------------------------------------------------------------
...
% IMPORTANT: THIS NEEDS TO BE REMOVED. IT MEANS SOMETHING WENT WRONG IN
% SOME TEXTURE CALCULATIONS
matData(isnan(matData(:))) = 0;


% GETTING BOOTSTRAP SAMPLES TO BE USED FOR ALL EXPERIMENTS
bootSam = ceil(nInst .* rand(nInst,nBoot));
...
if row <= nScan*nText
    if numel(paramSize) == 3 % SEPARATE scans
    ...
    elseif numel(paramSize) == 4 % FUSED scans
        ...
        string = [cellNames{row,1},'(',name,'Inv=','NoInv',',',name,'w=',num2str(paramAll{1}(indFinal(1)),'%.2f'),',R=',num2str(1,'%.2f'),',Scale=',num2str(paramAll{2}(indFinal(2))),',Quant.algo=',paramAll{3}{indFinal(3)},',Ng=',num2str(paramAll{4}(indFinal(4))),')--',cellNames{row,2}];
    end
