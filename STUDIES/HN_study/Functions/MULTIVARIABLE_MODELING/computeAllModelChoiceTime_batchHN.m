function computeAllModelChoiceTime_batchHN(pathExperiments,expNumber,maxOrder,nBoot,nBatch,matlabPATH,seed)
% -------------------------------------------------------------------------
...
save('workspace','pathSet','pathModels','training','param','maxOrder','nBoot','seed'), pause(5);
for i = 1:nBatch
    nameScript = ['batch',num2str(i),'_script.m'];
    fid = fopen(nameScript,'w');
    fprintf(fid,'load(''workspace'')\n');
    for j = 1:numel(param{i})
        fprintf(fid,['computeAllModelChoiceTime_HN(pathSet,pathModels,training,param{',num2str(i),'}{',num2str(j),'},maxOrder,nBoot,seed)\n']);
    end
