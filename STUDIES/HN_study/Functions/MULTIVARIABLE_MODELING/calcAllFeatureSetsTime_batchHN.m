function calcAllFeatureSetsTime_batchHN(pathMINE,pathExperiments,expNumber,setSize,alpha,delta,nBoot,nBatch,matlabPATH,seed)
% -------------------------------------------------------------------------
% function calcAllFeatureSets_batchHN(pathMINE,pathExperiments,nExperiments,setSize,alpha,delta,nBoot,nBatch,matlabPATH)
% -------------------------------------------------------------------------
% DESCRIPTION: 
...
% given as inputs. See ref. [1,2] for more details.
% -------------------------------------------------------------------------
% REFERENCES:
% [1] Vallieres, M. et al. (2015). FDG-PET/CT radiomics models for the if ~RandStream.getGlobalStream.Seed
...
% 3. expNumber: Number of the single experiment to perform
%                  --> Ex: 4
...
% 10. seed: Numerical number to use as seed for bootstrapping experiment
%           --> Ex: 54288
%
...
cd(fullfile(pathExperiments,['Experiment',num2str(expNumber)])), load('training'), mkdir('FSET'), cd('FSET'), pathSet = pwd; 
...
for i = 1:nBatch
    ...
    for j = 1:numel(param{i})
        fprintf(fid,['calcAllFeatureSetsTime_HN(pathSet,training,pathMINE,param{',num2str(i),'}{',num2str(j),'},setSize,alpha,delta,nBoot,seed,',num2str(i),')\n']);
    end
