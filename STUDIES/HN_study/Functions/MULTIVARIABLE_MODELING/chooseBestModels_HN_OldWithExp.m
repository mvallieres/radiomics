function chooseBestModels_HN_OldWithExp(pathExperimentsBinary,pathExperimentsTime,nExp,fSetNames,nameOutcomes,metric,maxOrder)
% -------------------------------------------------------------------------
...
fprintf('\n')
for exp = 1:nExp
    cd(pathExperimentsTime), pathExperimentTime = fullfile(pathExperimentsTime,['Experiment',num2str(exp)]);
    cd(pathExperimentTime), mkdir('FINAL_MODELS'), cd('FINAL_MODELS'), pathFinalModelsTime = pwd;
    cd(pathExperimentsBinary), pathExperimentBinary = fullfile(pathExperimentsBinary,['Experiment',num2str(exp)]);
    ...
    for o = 1:nOutcomes
        ...
        fprintf('\n======== DISPLAYING PREDICTION RESULTS FOR "EXPERIMENT %u", "%s" OUTCOME, "%s" METRIC  ========\n',exp,nameOutcomes{o},metric)
        for f = 1:nType
