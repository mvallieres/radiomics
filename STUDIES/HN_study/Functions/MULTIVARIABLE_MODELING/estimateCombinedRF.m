function [indClinic,bestCost] = estimateCombinedRF(pathRF,nameOutcome,fSetName,testComb,testCost,nSplit,testSplit,nBoot,seed,output)
...
for c = 1:nCost
    cost = testCost(c);
    for t = 1:nComb
        tableComb = [text,training.clinical.table(:,testComb{t})];
        cat = logical([zeros(1,nText),training.clinical.categories(testComb{t})]);
