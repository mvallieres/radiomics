function computeModelCoefficientsTimeAerts_batchHN(pathExperimentTime,nBatch,matlabPATH,seed)
% -------------------------------------------------------------------------
...
cd(pathExperimentTime), load('training')
cd('FINAL_MODELS'), pathFinalModels = pwd;
mkdir('batchLog_CoeffSign'), cd('batchLog_CoeffSign'), pathBatch = pwd; cd(pathFinalModels)
...
for o = 1:nOutcomes
    ...
    for f = 1:nFset
        mkdir([setNames{f},'sign']), cd([setNames{f},'sign'])
        finalModel = struct;
...
save('workspace','pathFinalModels','training','param','pathBatch','seed'), pause(3)
for i = 1:nBatch
    ...
    for j = 1:numel(param{i})
        fprintf(fid,['cd(fullfile(pathFinalModels,param{',num2str(i),'}{',num2str(j),'}{2},[param{',num2str(i),'}{',num2str(j),'}{1},''sign'']))\n']);
        fprintf(fid,'load(''finalModel'')\n');
        fprintf(fid,['fprintf(''COMPUTING THE COX REGRESSION COEFFICIENTS OF THE FINAL MODEL OF "',param{i}{j}{2},'" OUTCOME, "',param{i}{j}{1},'" FEATURE SET ... '')']);
        fprintf(fid,'\n');
        fprintf(fid,['[coeff,response,modelCI,medianHR] = computeModelCoefficientsTime_HN(finalModel.Data,training.',param{i}{j}{2},'.timeToEvent,1 - training.',param{i}{j}{2},'.outcome,seed);\n']);
        fprintf(fid,['fprintf(''DONE!\\n'')']);
        fprintf(fid,'\n');
        fprintf(fid,'save(''coeff'',''coeff''), save(''response'',''response''), save(''modelCI'',''modelCI''), save(''medianHR'',''medianHR'')\n');
    end
