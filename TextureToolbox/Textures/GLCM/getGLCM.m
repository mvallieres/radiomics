				...
				for J2 = j_min:j_max
					for K2 = k_min:k_max
						if I2 == i && J2 == j && K2 == k
							...
							if (abs(I2-i)+abs(J2-j)+abs(K2-k))==2 
                                GLCM(val_q3,val_neighbor) = GLCM(val_q3,val_neighbor) + sqrt(2); % Discretization length correction (M. Vallieres)
                            else
                                if (abs(I2-i)+abs(J2-j)+abs(K2-k))==3
                                    GLCM(val_q3,val_neighbor) = GLCM(val_q3,val_neighbor) + sqrt(3); % Discretization length correction (M. Vallieres)
                                else 
